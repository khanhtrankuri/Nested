...
class PrototypeContextModulator(nn.Module):
    ...
    def forward(self, feat: torch.Tensor, fast_memory: torch.Tensor, slow_memory: torch.Tensor):
        ...
        out = self.refine(out)
        return out, fast_attn, slow_attn, fast_ctx, slow_ctx


class PrototypeMemoryUpdater(nn.Module):
    def __init__(self, feat_dim: int, memory_dim: int, num_prototypes: int, hidden_dim: int = 128):
        super().__init__()
        self.pool = nn.AdaptiveAvgPool2d((1, 1))
        self.num_prototypes = num_prototypes
    ...
    def forward(
        ...
        fast_attn: torch.Tensor,
        slow_attn: torch.Tensor,
        ...
        delta_proto = write * fast_attn.unsqueeze(-1) * delta.unsqueeze(1)
        retain = 1.0 - 0.1 * erase * fast_attn.unsqueeze(-1)
        updated = fast_memory * retain + delta_proto
        return updated


class MemoryDecoderBlockV2(nn.Module):
    def __init__(
        self,
        in_channels: int,
        skip_channels: int,
        ...
        self.modulator = PrototypeContextModulator(out_channels, memory_dim, num_prototypes, hidden_dim)
    ...
    def forward(self, x, skip, edge, fast_memory, slow_memory):
        ...
        x, fast_attn, slow_attn, fast_ctx, slow_ctx = self.modulator(x, fast_memory, slow_memory)
        return x, {"fast_attn": fast_attn, "slow_attn": slow_attn, "fast_ctx": fast_ctx, "slow_ctx": slow_ctx}
...
class ReverseAttentionHead(nn.Module):
    def __init__(self, feat_channels: int, mid_channels: int = 32, out_channels: int = 1):
        super().__init__()
    ...
    def forward(self, feat: torch.Tensor, coarse_logits: torch.Tensor) -> torch.Tensor:
        ...
        return self.refine(torch.cat([feat, rev], dim=1))


class NBMPolypNetV2(nn.Module):
    def __init__(
        self,
        in_channels: int = 3,
        out_channels: int = 1,
        ...
        self.dec4 = MemoryDecoderBlockV2(c * 16, c * 8, c * 8, c * 8, memory_dim, num_prototypes, updater_hidden_dim)
        self.dec3 = MemoryDecoderBlockV2(c * 8, c * 4, c * 4, c * 4, memory_dim, num_prototypes, updater_hidden_dim)
        self.dec2 = MemoryDecoderBlockV2(c * 4, c * 2, c * 2, c * 2, memory_dim, num_prototypes, updater_hidden_dim)
        self.dec1 = EdgeAwareDecoderBlock(c * 2, c, c, c)

        self.coarse_head = nn.Conv2d(c, out_channels, kernel_size=1)
        self.refine_head = ReverseAttentionHead(c, mid_channels=c // 2, out_channels=out_channels)
        ...
        self.updaters = nn.ModuleDict(
            {
                stage: PrototypeMemoryUpdater(channels, memory_dim, num_prototypes, updater_hidden_dim)
    ...
    def init_memory(self, batch_size: int, device, dtype=torch.float32, from_slow: bool = True, noise_std: float = 1e-3, slow_scale: float = 0.2):
        memory = {}
        for stage in STAGE_KEYS:
            if from_slow:
    ...
    @torch.no_grad()
    def update_slow_memory(self, task_memory_summary: Dict[str, torch.Tensor], momentum: float | Dict[str, float] = 0.05, max_norm: float = 2.5):
        for stage in STAGE_KEYS:
    ...
    @torch.no_grad()
    def update_memory(self, memory_features: Dict[str, torch.Tensor], memory: Dict[str, torch.Tensor], update_signals: Dict[str, torch.Tensor], attention_cache: Dict[str, Dict[str, torch.Tensor]]) -> Dict[str, torch.Tensor]:
    ...
    @torch.no_grad()
    def predict_proba(self, x: torch.Tensor, memory: Optional[Dict[str, torch.Tensor]] = None, threshold: float = 0.5, use_memory: bool = True) -> torch.Tensor:
        self.eval()
        logits = self.forward(x, memory=memory, use_memory=use_memory)["logits"]
        return torch.sigmoid(logits)

    @torch.no_grad()
    def predict_mask(self, x: torch.Tensor, memory: Optional[Dict[str, torch.Tensor]] = None, threshold: float = 0.5, use_memory: bool = True) -> torch.Tensor:
        return (self.predict_proba(x, memory=memory, threshold=threshold, use_memory=use_memory) > threshold).float()
